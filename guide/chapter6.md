...
- [第一章 命题逻辑](https://hackmd.okfn.de/NojvGyLsSim2r4Uya0G9oQ)
- [第二章 一阶逻辑](https://hackmd.okfn.de/A-jR2zxORaOTEojZ1d13Eg)
- [第三章 皮亚诺算术](https://hackmd.okfn.de/TZF3W_x2QNaH1p25X7QKOA)
- [第四章 ZFC](#)
- [第五章 数系](#)
- [第六章 类型论](#Deductrium-攻略第六章-类型论)
- - [解释类型](#解释类型)
- - [开始](#开始)
- - [函数作用](#函数作用)
- - [证明助手](#证明助手)
...
\P{\True}\True
$$
这代表一个输入为$\True$类型，输出也为$\True$类型的函数。
函数是用$\L$构造的，输入时用`L`
给出几个可行的解：
$\L x:\True.x$
$\L x:\True.\true$
...
case在证明积类型时可以拆分左右，类似lean的constructor策略
...
`what := ind_Bool (Lx:Bool.ind_Bool (Lx:Bool.U1) (U) (U->U) x) True (Lx:U.True)`
+150µg推理素
...
$$
\P{x:\nat}\P{Πy:nat}\eqs{(\add{x}{y})}{(\add{y}{x})}
$$
